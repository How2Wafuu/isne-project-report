...
This appendix introduces the Wazuh Rule Syntax Glossary and presents the core configuration files and custom XML rule sets engineered to align the Wazuh SIEM with the HAIT+ compliance framework.

\section{Wazuh Rule Syntax Glossary}
\label{app:syntax-glossary}
To ensure the custom rules engineered for this project are easily understandable and maintainable by hospital IT staff, the following core XML tags utilized in the Wazuh decoding engine are defined below:
...
\begin{table}[h!]
    ...
    \begin{tabular}{|p{0.25\textwidth}|p{0.70\textwidth}|}
        \hline
        \textbf{XML Tag} & \textbf{Engineering Function / Description} \\
        \hline
        \texttt{<rule id="" level="">} & Initializes a custom rule. Custom IDs must be 100000 or higher. The \texttt{level} (0-15) dictates the severity of the alert on the dashboard. \\
        \hline
        \texttt{<if\_sid>} & Establishes a Parent-Child hierarchy. The custom rule will \textit{only} trigger if the specified parent Rule ID has already fired (crucial for decoder overrides). \\
        \hline
        \texttt{<match>} & Performs a fast, basic string search within the log payload. It triggers if the exact text is found. \\
        \hline
        \texttt{<regex type="pcre2">} & Utilizes Perl Compatible Regular Expressions (PCRE2) for advanced pattern matching. Used extensively in the Command-to-Rule pipeline to parse dynamic numbers (like CPU load or Disk Space percentages). \\
        \hline
        \texttt{<description>} & The human-readable alert text that is directly displayed to administrators on the Wazuh Security Events dashboard. \\
        \hline
        \texttt{<group>} & Assigns category tags to the rule. In this project, the \texttt{ha\_it\_plus} tag is appended to allow immediate filtering for healthcare compliance audits. \\
        \hline
        \texttt{<mitre> \textless id\textgreater} & Maps the specific alert directly to the MITRE ATT\&CK framework (e.g., T1003.008 for OS Credential Dumping), providing instant threat intelligence context. \\
        \hline
    \end{tabular}
    \caption{Core XML tags utilized in the custom HAIT+ Wazuh ruleset.}
    \label{tab:xml_glossary}
\end{table}
